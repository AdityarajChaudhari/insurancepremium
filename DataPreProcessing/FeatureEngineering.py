...
class PreProcessor:
    ...
    def featureencoding(self):
        ...
        try:
            self.log_object.create_log_file('Coverting Categorical Values from Feature Column to Numerical values.')
            encoded_data = pd.get_dummies(self.loader.get_data()[['sex', 'smoker', 'region']], drop_first=True)
            self.log_object.create_log_file('Coverted Categorical Values from Feature Column to Numerical values.')
